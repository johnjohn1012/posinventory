...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Verify employee is Cashier or Stock Clerk
        $verify_query = "SELECT j.job_name 
                        FROM tbl_employee e
                        INNER JOIN tbl_jobs j ON e.job_id = j.job_id
                        WHERE e.employee_id = ? AND j.job_id IN (1, 2)";
        ...
        if ($verify_result->num_rows === 0) {
            throw new Exception("Unauthorized: Cannot create user account for Cashier or Stock Clerk roles");
        }
