...
$stmt = $conn->prepare("SELECT job_id, job_name FROM tbl_jobs WHERE job_name != 'Main Admin' ORDER BY job_name");
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($first_name) || empty($last_name) || empty($email) || empty($gender) || 
        ...
        if ($result->num_rows > 0) {
            ...
            // Verify the selected job is not Main Admin
            $stmt = $conn->prepare("SELECT job_id FROM tbl_jobs WHERE job_id = ? AND job_name != 'Main Admin'");
